#!/bin/bash
#
# Install flash-attention Python environment: dependencies and wheel package.
...
run_pip() {
    if [ "$DEBUG" -eq 1 ]; then
        ...
        local log_file
        log_file="$(mktemp /tmp/flash-attention_pip_XXXXXX.log)"
...
echo ""
echo "============================================"
echo "  flash-attention Environment Installer"
echo "============================================"
...
WHEELS=($REPO_ROOT/dist/flash_attn*.whl)
if [ ! -f "${WHEELS[0]}" ]; then
    echo -e "${RED}[ERROR]${NC} No flash-attention wheel found in $REPO_ROOT/dist/"
...
next_step "Install flash-attention wheel"
log_info "Wheel package(s):"
ls -lh "${WHEELS[@]}"
echo ""
log_warn "You may be prompted for your sudo password again."
run_pip install "${WHEELS[@]}"
log_ok "flash-attention installed: $(python3 -c 'import flash_attn; print(flash_attn.__file__)' 2>/dev/null || echo '(import check skipped)')"
